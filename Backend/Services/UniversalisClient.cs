...
public class UniversalisClient : HttpClient
{
    ...
    private UniversalisClient()
    {
        ...
        foreach (var productHeader in new[]
                 {
                     new ProductInfoHeaderValue("WildWolfUniversalisBot", version?.InformationalVersion ?? "0.0.0"),
                     new ProductInfoHeaderValue(".NET", netVersion?.FrameworkDisplayName?.Split(' ')[1]),
                     new ProductInfoHeaderValue("CoreCLR", assembly.ImageRuntimeVersion)
    ...
    public static async Task<UniversalisClient> Init()
    {
        ...
        foreach (var world in await client.GetWorlds()) await client.GetTaxRates(world.Id);
        return client;
    }
    ...
    public async Task<Listing> GetListing(string worldId, uint itemId, ListingQuery? query = null)
    {
        ...
        if (_listingsCache.TryGetValue(worldId + itemId, out var cachedListing) &&
            cachedListing.Item2 > DateTime.UtcNow.AddHours(-3))
    ...
    public async Task<History> GetHistory(string worldId, uint itemId, HistoryQuery? query = null)
    {
        ...
        if (_historiesCache.TryGetValue(worldId + itemId, out var cachedHistory) &&
            cachedHistory.Item2 > DateTime.UtcNow.AddHours(-3))
