...
public class StatusController : ControllerBase
{
    ...
    {
        var statuses = _client.GetSheet<Status>()?.Where(t => !string.IsNullOrWhiteSpace(t.Name) && t.Icon != 0).Select(t => (StatusHttp)t).ToList();
        if (statuses == null)
            return NotFound();
        return Ok(statuses);
    }
...
public class StatusHttpConverter : JsonConverter<StatusHttp>
{
    ...
    public override void Write(Utf8JsonWriter writer, StatusHttp value, JsonSerializerOptions options)
    {
        ...
        writer.WriteString("description", value.Description); // currently will return bad unicode data due to not using the new SeString parser that will be added after 3.15.2
