...
public record History
{
    ...
    public ProcessedSale GetPriceHistory()
    {
        return new ProcessedSale(Sales.Select(t => new SanitizedSale(t)));
    }
}
...
public static class Extensions
{
    ...
    public static Dictionary<TK, TV> ToDictionaryOrdered<T, TK, TV>(this IEnumerable<T> values, Func<T, TK> key,
        Func<T, TV> value) where TK : notnull
    {
        return values.ToDictionary(key, value, EqualityComparer<TK>.Default).OrderBy(t => t.Key)
...
#pragma warning restore
        ...
        var avgSalesNq = sales.Where(sale => sale.AveragePriceNq.HasValue)
            .ToDictionaryOrdered(sale => sale.Date.ToOADate(), sale => sale.AveragePriceNq!.Value);
        var avgSalesHq = sales.Where(sale => sale.AveragePriceHq.HasValue)
            .ToDictionaryOrdered(sale => sale.Date.ToOADate(), sale => sale.AveragePriceHq!.Value);
        var avg = plt.Add.Scatter(avgSales.Keys.ToArray(), avgSales.Values.ToArray(),
            Color.FromARGB((128 * 256) ^ (3 + 127 * 256 + 255)));
        avg.Label = "Average Price";

        if (avgSalesNq.Any())
        {
            var avgNq = plt.Add.Scatter(avgSalesNq.Keys.ToArray(), avgSalesNq.Values.ToArray(),
                Color.FromARGB((128 * 256) ^ (3 + 255 * 256)));
            avgNq.Label = "Average Price (NQ)";
        }

        if (avgSalesHq.Any())
        {
            var avgHq = plt.Add.Scatter(avgSalesHq.Keys.ToArray(), avgSalesHq.Values.ToArray(),
                Color.FromARGB((128 * 256) ^ (3 + 255)));
